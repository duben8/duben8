...
namespace structures
{
	...
	inline ArrayList<T>& ArrayList<T>::operator=(const ArrayList<T>& other)
	{
		...
		if (this != &other) { //skontrolujem ci nepriradujem sam seba
			size_ = other.size_;
			delete array_;
			array_ = new Array<T>(*other.array_); //iba znicim nase pole a zavolam kopirovaci konstruktor a dam donho array druheho
		}
		return *this;
	}
	...
	inline T & ArrayList<T>::operator[](const int index)
	{
		//TODO 03: ArrayList
		//throw std::exception("ArrayList<T>::operator[]: Not implemented yet.");
		DSRoutines::rangeCheckExcept(index, size_, "Invalid index at ArrayList<T>::operator[]"); //musim kontrolovat lebo array moze byt vacsi ako pocet platnych prvkov
		return (*array_)[index];
	}
	...
	inline void ArrayList<T>::insert(const T & data, const int index)
	{
		...
		DSRoutines::rangeCheckExcept(index, size_, "Invalid index at ArrayList<T>::insert ");
		if (array_->size() == size_) {
			enlarge();
		}
	...
	template<typename T>
	inline int ArrayList<T>::getIndexOf(const T & data) // parametrom je kopia tych dat
	{
		//TODO 03: ArrayList
	...
	inline void ArrayList<T>::clear()
	{
		...
		size_ = 0; //staci len napisat ze je prazdny , pole ma nezaujima absolutne
	}
